...
func systemPromptForRole(role Role, lang, progLang, ruleCatalog string, toolNames []string, flags ...bool) string {
	...
	case RolePlanner:
		...
		b.WriteString("- After reading the task, BEGIN your plan with a 'CONSTRAINTS:' section listing ALL explicit prohibitions and constraints found in the task text. These override any other consideration. Every subsequent step, technology choice, and verification command MUST comply with these constraints.\n\n")
		...
		b.WriteString("- Create a numbered step-by-step plan. Each step MUST use this format:\n")
		b.WriteString("  N. <title>\n")
		b.WriteString("     file: <path>\n")
		b.WriteString("     context: <all details the executor needs to complete this step independently: data structures, field names, formats, requirements, constraints. The executor cannot see the original task>\n")
		...
		b.WriteString("- Verification section MUST only include commands that match the actual project stack and task scope. Do NOT invent commands for tools/servers not present in the project.\n")
	...
	case RoleExtractor:
		...
		b.WriteString("Your job is to convert raw planner output into a strict JSON plan.\n\n")
		b.WriteString("Input may be one of two kinds:\n")
		b.WriteString("1. IMPLEMENTATION PLAN — numbered steps describing files to create/edit/delete (action items, not issues).\n")
		b.WriteString("2. REVIEW / ANALYSIS — scattered review comments, findings, and suggestions.\n")
		b.WriteString("You MUST handle BOTH kinds. An implementation plan with concrete steps is NOT \"no issues found\" — convert each step into a plan item.\n\n")
		...
		b.WriteString("{\n")
		b.WriteString("  \"context\": \"<detailed description of the goal preserving ALL specifics: data structures, field names, formats, requirements, constraints>\",\n")
		b.WriteString("  \"plan\": [\n")
		b.WriteString("    {\n")
		b.WriteString("      \"file\": \"<relative/path/to/file.ext>\",\n")
		b.WriteString("      \"line\": <integer, 0 for new files>,\n")
		b.WriteString("      \"severity\": \"blocker\" | \"major\" | \"minor\" | \"task\",\n")
		b.WriteString("      \"what\": \"<short description of what to do>\",\n")
		b.WriteString("      \"context\": \"<all details the executor needs to complete this item independently: data structures, formats, requirements, constraints. The executor cannot see the original task>\",\n")
		b.WriteString("      \"why\": \"<why this is needed>\",\n")
		b.WriteString("      \"fix\": \"<concrete action or fix>\",\n")
		b.WriteString("      \"before\": \"<existing code snippet (optional, reviews only)>\",\n")
		b.WriteString("      \"after\": \"<corrected code snippet (optional, reviews only)>\",\n")
		...
		b.WriteString("- SEVERITY: for review findings use `blocker`/`major`/`minor`. For implementation steps use `task`.\n")
		b.WriteString("- Describe WHAT must be done concisely. Describe WHY separately.\n")
		b.WriteString("- Do NOT add items not present in the original plan. Do NOT invent work.\n")
		b.WriteString("- Deduplicate: if the same file appears in multiple steps with related actions, emit it once.\n")
		b.WriteString("- Group by file: if multiple actions affect the same file, emit ONE plan item per file. Combine all actions for that file in the `what` field, separated by semicolons.\n")
		...
		b.WriteString("- `verification` — list concrete shell commands to verify correctness (e.g. `make build`, `make lint`, `make test-unit`). Include at least build and lint. ONLY include commands that match the actual project stack — do NOT invent commands for tools/servers not present in the project.\n")
		b.WriteString("- Output `{\"context\": \"LGTM — no issues found\", \"plan\": []}` ONLY when the input explicitly states there is nothing to do. An implementation plan with steps is NEVER \"no issues\".\n")
		if lang == progLangGo {
	...
	case RoleStructurer:
		...
		b.WriteString("{\n")
		b.WriteString("  \"context\": \"<comprehensive summary: project purpose, tech stack, architecture, key requirements, constraints AND explicit prohibitions from the task. Multiple paragraphs are OK — the executor sees ONLY this context, not the original task>\",\n")
		b.WriteString("  \"steps\": [\n")
		b.WriteString("    {\n")
		b.WriteString("      \"id\": <integer, starting at 1>,\n")
		b.WriteString("      \"title\": \"<short imperative sentence>\",\n")
		b.WriteString("      \"context\": \"<ALL details from the task needed to complete THIS step: data structures, field names, formats, types, constraints, visual/behavioral requirements, related components. Minimum 2-3 sentences even for simple steps. The executor sees ONLY this context — not the original task>\",\n")
		b.WriteString("      \"files\": [\"<relative file path>\", ...],\n")
		b.WriteString("      \"action\": \"edit\" | \"create\" | \"delete\" | \"run\",\n")
		b.WriteString("      \"rationale\": \"<why this step is needed>\",\n")
		b.WriteString("      \"depends_on\": [<id>, ...],\n")
		b.WriteString("      \"rule_names\": [\"<rule name referenced in the plan>\", ...],\n")
		...
		b.WriteString("- `context` is MANDATORY and must be DETAILED. Include ALL specifics from the original task that the executor needs for this step: field names, data types, formats, validation rules, business logic, visual/behavioral requirements. The executor cannot see the original task — this is its only source of requirements. Even simple steps need project-level context. BAD: 'Create .gitignore file'. GOOD: 'Create .gitignore for a Python/HTMX project using Flask: exclude __pycache__/, .env, .venv/, node_modules/, *.pyc, .DS_Store. The project serves HTMX partials from templates/ and static files from static/.'.\n")
		b.WriteString("- `rule_names` is optional. Include rule names mentioned in the plan text for this step (e.g. \"service\", \"validator\", \"tests\"). Do not fabricate rule names.\n")
