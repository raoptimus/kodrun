...
Go is a new language that borrows ideas from existing languages but has unusual properties that make effective Go programs different from programs written in related languages. A straightforward translation of a C++ or Java program into Go is unlikely to produce satisfactory results—Java programs are written in Java, not Go. Thinking about the problem from a Go perspective could produce a successful but quite different program.

To write Go well, it's important to understand its properties and idioms, as well as established conventions for programming in Go, such as naming, formatting, program construction, and so on.

This document gives tips for writing clear, idiomatic Go code. It augments the language specification, the Tour of Go, and How to Write Go Code, all of which should be read first.

**Note (January 2022):** This document was written for Go's release in 2009 and has not been updated significantly since. Although it is a good guide to understanding how to use the language itself, thanks to the stability of the language, it says little about the libraries and nothing about significant changes to the Go ecosystem since it was written, such as the build system, testing, modules, and polymorphism. There are no plans to update it, as so much has happened and a large growing set of documents, blogs, and books describe modern Go usage. Effective Go continues to be useful, but readers should understand it is far from a complete guide.

### Examples

The Go package sources serve as core library and examples of how to use the language. Many packages contain working, self-contained executable examples you can run directly from the go.dev website. If you have a question about how to approach a problem or how something might be implemented, the documentation, code, and examples in the library can provide answers, ideas, and background.

## Formatting

Formatting issues are the most contentious but the least consequential. People can adapt to different formatting styles, but it's better if they don't have to. Go takes an unusual approach by letting the machine handle most formatting issues.

The `gofmt` program (also available as `go fmt`, which operates at the package level) reads a Go program and emits the source in a standard style of indentation and vertical alignment, retaining and reformatting comments as necessary. If you want to know how to handle a new layout situation, run `gofmt`; if the answer doesn't seem right, rearrange your program (or file a bug about `gofmt`), don't work around it.
...
Use tabs for indentation; `gofmt` emits them by default. Use spaces only if you must.
...
Go needs fewer parentheses than C and Java: control structures (`if`, `for`, `switch`) do not have parentheses in their syntax. The operator precedence hierarchy is shorter and clearer, so:

```go
x<<8 + y<<16
```

means what the spacing implies, unlike in other languages.
...
- Don't worry about collisions *a priori*. The package name is only the default name for imports; it need not be unique across all source code
...
The rule is: If the last token before a newline is an identifier (which includes words like `int` and `float64`), a basic literal such as a number or string constant, or one of the tokens:
...
needs no semicolons. Idiomatic Go programs have semicolons only in places such as `for` loop clauses, to separate the initializer, condition, and continuation elements. They are also necessary to separate multiple statements on a line.
...
Note: In Go the scope of function parameters and return values is the same as the function body, even though they appear lexically outside the braces that enclose the body.
...
Because `recover` always returns `nil` unless called directly in a deferred function, deferred code can call functions that themselves use `panic` and `recover` without failing. For example, the deferred function in `safelyDo` might call a logging function before calling `recover`, and that logging code would run unaffected by the panicking state.

With our recovery pattern in place, the `do` function (and anything it calls) can get out of any bad situation cleanly by calling `panic`. We can use that idea to simplify error handling in complex software. Let's look at an idealized version of a `regexp` package, which reports parsing errors by calling `panic` with a local error type.

```go
// Error is the type of parse errors; it satisfies the error interface.
type Error string
...
type parser struct {
    ...
    pos    int    // next character position
}

// parseRegexp parses a regular expression.
func (p *parser) parseRegexp() *Regexp {
    ch := p.nextChar()
    switch {
    case ch == '|':
        return p.newAlternation()
    case ch == '(':
        return p.parseGroup()
    ...
}

// parseError panics with an error.
func (p *parser) parseError(err string) {
    panic(Error(err))
}
```

If `parseRegexp` panics, the recovery will catch the `Error`, assign it to the error result through the deferred function, and the function will return as if the parser had called an error function. If no panic occurs, the panic recovery code does nothing and the function returns the compiled result normally.

With error handling set up like this, the `error` method (which is just a method on a value of type `Error`) needs only to report the parse error, without cluttering the code that does the actual parsing.

## A web server

Let's finish with a complete Go program. This is a web server that shows an HTML page and responds to requests:
...
func view(w http.ResponseWriter, r *http.Request) {
    ...
    fmt.Fprintf(w, "<h1>%s</h1><pre>%s</pre>", title, body)
}
```

This web server listens on a specified port and provides two endpoints: `/` displays an HTML front page, and `/view/` displays the contents of requested files. It demonstrates several Go features including flag parsing, HTTP handling, and file I/O with proper error handling.